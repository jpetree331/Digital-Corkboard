...
4. Add a server environment variable named **`BOARD_PASSWORD`**: choose a long, random password of **at least 16 characters**. Share it privately with your friend. Do not put it in this repository or prefix either secret with `VITE_`. This is only the starting password: whoever unlocks the workspace can replace it from the gear menu inside the app, after which `BOARD_PASSWORD` is no longer accepted.
