...
4. Add a server environment variable named **`BOARD_PASSWORD`**: choose a long, random password of **at least 12 characters**. Share it privately with your friend. Do not put it in this repository or prefix either secret with `VITE_`. This is only the starting password: whoever unlocks the workspace can replace it from the gear menu inside the app, after which `BOARD_PASSWORD` is no longer accepted.
...
The API verifies a signed, HttpOnly, SameSite cookie before database or attachment access. Production cookies are Secure and expire after seven days. Login and password-change attempts are limited in the database. The database URL and password stay on the server.

**Changing the password.** Click the gear (⚙) in the top-right ribbon and choose a new password of at least 12 characters. The new password is stored in Neon as a salted scrypt hash in `corkboard_settings`; the hosting account cannot read it back, and `BOARD_PASSWORD` stops working. Changing the password signs out every other device while keeping the current one unlocked. Until a password has been chosen in the app, `BOARD_PASSWORD` is used and changing it plus redeploying invalidates previous sessions.
...
- **Set BOARD_PASSWORD…**: configure a password of at least 12 characters in the correct Vercel environment, then redeploy.
- **Forgotten password**: the host cannot recover a password chosen in the app. To reset, run `delete from corkboard_settings;` in the Neon SQL editor; the workspace then accepts `BOARD_PASSWORD` again and a new password can be chosen from the gear menu.
